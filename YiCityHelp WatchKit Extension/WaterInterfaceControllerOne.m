...
#import "WaterInterfaceControllerOne.h"

@interface WaterInterfaceControllerOne ()
@property (unsafe_unretained, nonatomic) IBOutlet WKInterfaceLabel *updateLabel;
@property (unsafe_unretained, nonatomic) IBOutlet WKInterfaceLabel *GMSJLabel;
//PH标签表
@property (unsafe_unretained, nonatomic) IBOutlet WKInterfaceLabel *GMSJWaterLabel;
@property (nonatomic,strong)NSArray *riverArr;
@end

@implementation WaterInterfaceControllerOne
...
- (void)willActivate {
    ...
    {
    //高锰酸钾赋值
    [self.GMSJLabel setText:[NSString stringWithFormat:@"%@",self.riverArr[0][@"山东济南泺口"][@"permangan"]]];
    //高锰酸钾水质赋值
    [self.GMSJWaterLabel setText:[NSString stringWithFormat:@"%@",self.riverArr[0][@"山东济南泺口"][@"permanganquality"]]];
    //更新时间表
